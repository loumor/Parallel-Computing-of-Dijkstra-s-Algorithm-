...
void generateEmptyGraph(long N, graph *G)
{
    ...
    //#pragma omp parallel for schedule(runtime)  // Works but not needed
	for(i = 0; i < G->N; i++){
		G->node[i] = &temp[i*G->N];
	}
}
...
void generateGraph(long N, int randInit, graph *G, char debug, int nthreads)
{
	long linkCnt;
	long i,j;
	long t;
    ...
    omp_set_schedule(omp_sched_auto, G->N/nthreads);
	...
	for(i = 0; i< G->N; i++)
	{
     ...
     //#pragma omp parallel for schedule(runtime) private(t) // Does not work
		for(j = 0; j < G->N; j++)
		{
...
long getNextNode(graph* G)
{
    ...
    //#pragma omp parallel for schedule(runtime) shared(minD, nextNode) // Does not work
	for(i = 0; i<G->N; i++){
